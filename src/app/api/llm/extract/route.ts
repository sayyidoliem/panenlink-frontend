 ...
 * Returns a FarmerExtraction JSON object for post-load autofill.
...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${LLM_BASE}/extract`, {
      method: "POST",
    ...
    console.error("[/api/llm/extract] LLM service unavailable:", err);
    return NextResponse.json(
      { error: "LLM service tidak tersedia." },
      { status: 503 }
    );
  }
}
